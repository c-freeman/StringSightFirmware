 ...
 * Make sure to match the chosen payload_port with the onboard sensors
 * available. e.g. portSchema payload_port = PORT3; requires a RAK1901 or
 * RAK1906 to acquire the temperature. You also need to indicate which sensor to
 * use in SensorHelper.h by setting the USE_RAK1901, etc. to true.
 *
 * @version 0.1
 * @date 2021-08-24
 *
...
// PortSchema.h E.g. port 3: battery voltage + temperature This example uses the
// RAK1901 for temp - change initSensors to switch to the 1906
portSchema payload_port = PORT10;

// Sensor reading interval in [ms] = 10 seconds.
const int sensor_reading_interval = 10000;
...
void setup() {
    ...
    log(LOG_LEVEL::INFO,
        "\n========================================"
        "\nWelcome to Current Sensor with LoRaWAN"
        "\n========================================");
 ...
 */
void loop() {
    // every sensor_reading_interval ms check if connected and then send sensor
    // payload
...
void fillPayload(void) {
    ...
    log(LOG_LEVEL::INFO,
        "c: %.2f %% ",
        sensor_data.current_A.value);

        //         "c: %.2f %% | b: %.2f %% ",
        // sensor_data.current_mv.value, sensor_data.battery_mv.value);

        // "c: %.2f %% | b: %.2f %% | t: %.2f C | h: %.2f %% | p: %lu Pa | g: %lu | l: %.5f, "
        // "%.5f",
        // sensor_data.current_mv.value, sensor_data.battery_mv.value, sensor_data.temperature.value, sensor_data.humidity.value, sensor_data.pressure.value,
